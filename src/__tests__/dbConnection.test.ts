...
describe('Database Connection Handling', () => {
  ...
  it('should log an error and exit if database connection fails at startup', async () => {
    ...
    const originalConnectPrisma = vi.importActual('../utils/prisma').then(m => m.connectPrisma);
    (vi.mocked(await originalConnectPrisma) as vi.Mock).mockImplementation(async () => {
      try {
    ...
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to connect to the database.'));
