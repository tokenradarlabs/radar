...
describe('fetchTokenPriceChange', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return token price change on successful fetch', async () => {
    const mockResponse = {
      ok: true,
      json: () => Promise.resolve({ bitcoin: { usd_24h_change: 1.23 } }),
    };
    (fetchWithRetry as vi.Mock).mockResolvedValue(mockResponse as Response);

    const result = await fetchTokenPriceChange('bitcoin');
  ...
  it('should return null if API response is invalid', async () => {
    const mockResponse = {
      ok: true,
      json: () => Promise.resolve({ bitcoin: { usd: 50000 } }), // Missing usd_24h_change
    };
    (fetchWithRetry as vi.Mock).mockResolvedValue(mockResponse as Response);
  ...
  it('should return null if token data is not found in response', async () => {
    const mockResponse = {
      ok: true,
      json: () => Promise.resolve({ ethereum: { usd_24h_change: 4.56 } }),
    };
    (fetchWithRetry as vi.Mock).mockResolvedValue(mockResponse as Response);

    const result = await fetchTokenPriceChange('bitcoin'); // Requesting bitcoin, but response has ethereum
    expect(result).toBeNull();
  });
});
