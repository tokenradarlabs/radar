...
describe("API Key Update Endpoint", () => {
  ...
  beforeEach(async () => {
    ...
    testApiKey = await prisma.apiKey.create({
      data: {
        key: 'rdr_' + Buffer.from('test-update-key-1234567890abcdef1234567890abcdef12345678').toString('hex').substring(0, 64),
  ...
  it('should update multiple API keys independently', async () => {
    ...
    const secondApiKey = await prisma.apiKey.create({
      data: {
        key: 'rdr_' + Buffer.from('test-update-key-2-34567890abcdef1234567890abcdef12345678').toString('hex').substring(0, 64),
